...
def _weighted_covariance_gpu(z: torch.Tensor, w: torch.Tensor, eta: torch.Tensor, lambda_reg: float) -> torch.Tensor:
    # z: (T, D), w: (T,), eta: (D,) - すべてGPU上
    # effective sample correction
    sum_w2 = (w ** 2).sum()
    alpha = 1.0 / max(1e-8, (1.0 - sum_w2).item())
    diff = z - eta[None, :]
    # (D,D) - einsumで効率的に計算
    G = alpha * torch.einsum('ti,tj,t->ij', diff, diff, w)
    # regularization
    G = G + lambda_reg * torch.eye(G.shape[0], dtype=G.dtype, device=G.device)
    return G
...
def run(config_path: str) -> None:
    ...
    for i, text in enumerate(tqdm(texts, desc="Extracting embeddings and metrics", unit="sample")):
        ...
        # pick last valid token position t* (seq_lenを先に計算)
        ...
        if attn.shape[1] > t_star and attn.shape[2] > t_star:
            w = attn[:, t_star, :].mean(dim=0)  # (T,)
        else:
            # フォールバック: 均等重み
            w = torch.ones(attn.shape[2], device=device) / attn.shape[2]
        ...
        eta = torch.einsum('t,td->d', w, z)  # (D,) - GPU上で計算
        G_theta = _weighted_covariance_gpu(z, w, eta, float(cfg["lambda_reg"]))  # (D,D) - GPU上で計算
